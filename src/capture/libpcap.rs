...
impl Capture<Libpcap> {
    ...
    pub fn link_type(&self) -> u16 {
        self.backend.pcap_file.as_ref().unwrap().get_datalink().0 as u16
    }

    pub fn new() -> Capture<Libpcap> {
        Capture {
            backend: Libpcap { pcap_file: None },
        }
    }
    ...
    pub fn next(&mut self) -> Result<Packet, Error> {
        match self.backend.pcap_file.as_mut().unwrap().next() {
