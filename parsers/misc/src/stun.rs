...
pub fn register_classify_rules(
    ...
    let mut dpi_rule = dpi::Rule::new(pattern! {r"^RSP/...STUN"});
    ...
    match_cbs.insert(rule_id, MatchCallBack::ProtocolName("stun".to_string()));

    let mut dpi_rule = dpi::Rule::new(pattern! {r"^((\x00\x01)|(\x0\x03)|(\x01\x01))\x00"});
